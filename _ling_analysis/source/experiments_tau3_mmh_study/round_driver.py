...
from .guidelines import GuidelineMemory, TaskObservation  # noqa: E402
from .tau3_binding import (  # noqa: E402
    EvalInvocation,
    load_per_task,
    parse_scores,
    run_eval,
...
class RoundDriver:
    def __init__(
        ...
        *,
        ...
        # Resolve the harness dir relative to THIS file so the package works under both
        # the local (`experiments/tau3_mmh`) and lab (`experiments_tau3_mmh`) layouts.
        self.config_dir = config_dir or (Path(__file__).resolve().parent / "tau3_harness")
        ...
        from meta_memory import build_embedding_engine
        ...
        self.memory = GuidelineMemory.from_snapshot(self.memory_path, embedding_provider=self.embedding) if (
    ...
    def _apply_evidence(self, round_id: int, observations: Sequence[TaskObservation],
        ...
        """
        from .proposer import FailureSignature
    ...
    def _proposer(self):
        from .proposer import LLMProposer, OfflineProposer
    ...
    def _requested_keys(tasks: Sequence[str]) -> set[str]:
        ...
        Only the FIRST colon is replaced. The benchmark names a task
        ``f"{domain}_{task_id}"`` and telecom task ids themselves contain colons
        (``...unseat_sim_card[PERSONA:Hard]``), so a blanket ``replace(":", "_")``
        corrupts telecom keys and trips the task-set verification -- which is how this
        was caught in practice.
        """
        return {t.replace(":", "_", 1) for t in tasks}
    ...
    def _load_per_task(self, invocation: EvalInvocation,
        ...
        """
        from .tau3_binding import _locate_scores
