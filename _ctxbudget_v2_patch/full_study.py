...
def inspect_record(record, trace_dir):
    ...
    for event in events:
        if event.get('type') == 'model_error':
            last_model_error = event
        if event.get('type') != 'agent_error':
            continue
        detail = str(event.get('error', ''))
        if 'HTTPStatusError' in detail and is_context_limit_error(last_model_error):
            # Native context exhaustion is an observed scored capability failure.
            continue
...
def capability_limit_errors(events):
    """Return response-backed native context-limit rejections for audit only.

    Such a rejection is a scored model/harness capability failure, never a
    transport retry.  The adapter records the server response body verbatim.
    """
    return [event for event in events
            if is_context_limit_error(event)]


def is_context_limit_error(event):
    return (isinstance(event, dict) and event.get('type') == 'model_error'
            and event.get('http_status_code') == 400
            and CONTEXT_LIMIT in str(event.get('response_body', '')).lower())
        ...
        for attempt in range(1, 4):
            ...
            for record, key in zip(records, keys):
                ...
                completed[key] = {'task': key, 'reward': float(reward), 'passed': float(reward) > 0,
                                  ...
                                  'capability_limit_errors':len(capability_limit_errors(events))}
